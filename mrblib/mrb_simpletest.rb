...
class SimpleTest
  ...
  def assert(str = 'Assertion failed', iso = '')
    ...
    begin
      ...
      if e.class.to_s == 'MRubyTestSkip'
        ...
        @asserts.push(self.assertion_string("#{e.class}: ", str, iso, e))
        @kill_test += 1
        self.t_print('X')
    end
    ensure
  ...
  def assert_equal(arg1, arg2 = nil, arg3 = nil)
    ...
    diff = self.assertion_diff(exp, act)
  ...
  def assert_not_equal(arg1, arg2 = nil, arg3 = nil)
    ...
    diff = self.assertion_diff(exp, act)
    self.assert_false(exp == act, msg, diff)
  end

  def assert_nil(obj, msg = nil)
    msg = "Expected #{obj.inspect} to be nil" unless msg
    diff = self.assertion_diff(nil, obj)
    self.assert_true(obj.nil?, msg, diff)
  end
  ...
  def assert_include(collection, obj, msg = nil)
    ...
    diff = "    Collection: #{collection.inspect}\n" +
           "        Object: #{obj.inspect}"
    self.assert_true(collection.include?(obj), msg, diff)
  end
  ...
  def assert_not_include(collection, obj, msg = nil)
    ...
    diff = "    Collection: #{collection.inspect}\n" +
           "        Object: #{obj.inspect}"
    self.assert_false(collection.include?(obj), msg, diff)
  end
  ...
  def assert_kind_of(cls, obj, msg = nil)
    ...
    diff = self.assertion_diff(cls, obj.class)
    self.assert_true(obj.kind_of?(cls), msg, diff)
  end
  ...
  def assert_float(exp, act, msg = nil)
    ...
    diff = self.assertion_diff(exp, act)
    self.assert_true check_float(exp, act), msg, diff
  end
  ...
  def report()
    ...
    self.t_print("Total: #{@total_test}\n")

    self.t_print("   OK: #{@ok_test}\n")
    self.t_print("   KO: #{@ko_test}\n")
    self.t_print("Crash: #{@kill_test}\n")
